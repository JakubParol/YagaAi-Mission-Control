...
provision_openclaw_auth() {
  ...
  if [[ ! -f "$dev_auth_path" ]]; then
    ...
    cp "$prod_auth_path" "$dev_auth_path"
    chown "$INSTALL_USER:$INSTALL_USER" "$dev_auth_path" 2>/dev/null || true
    chmod 600 "$dev_auth_path"
    echo "[INFO] Copied PROD auth to DEV: $dev_auth_path"
  fi

  # Update PROD env to point at the container-internal path
  if ! grep -q 'MC_API_OPENCLAW_DEVICE_AUTH_PATH' "$PROD_ENV_FILE" 2>/dev/null; then
    {
      echo ""
  ...
  if grep -q 'MC_API_OPENCLAW_GATEWAY_TOKEN' "$PROD_ENV_FILE" 2>/dev/null; then
  ...
  if grep -q 'MC_API_OPENCLAW_DEVICE_IDENTITY_PATH' "$PROD_ENV_FILE" 2>/dev/null; then
