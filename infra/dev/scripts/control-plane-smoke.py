...
def _get_run(base_url: str, run_id: str) -> dict[str, Any]:
    return _http_json(base_url, "GET", f"/v1/control-plane/runs/{urllib.parse.quote(run_id, safe='')}")
...
def _get_attempts(base_url: str, run_id: str) -> dict[str, Any]:
    return _http_json(
        base_url,
        "GET",
        f"/v1/control-plane/runs/{urllib.parse.quote(run_id, safe='')}/attempts?limit=20",
    )


def _get_metrics(base_url: str) -> dict[str, Any]:
    return _http_json(base_url, "GET", "/v1/control-plane/metrics")
...
with psycopg.connect(dsn) as conn:
    with conn.cursor() as cur:
        for run_id in run_ids:
            cur.execute('DELETE FROM control_plane_run_steps WHERE run_id = %s', (run_id,))
            cur.execute('DELETE FROM control_plane_run_timeline WHERE run_id = %s', (run_id,))
            cur.execute('DELETE FROM control_plane_runs WHERE run_id = %s', (run_id,))
        for correlation_id in correlations:
            cur.execute('DELETE FROM control_plane_processed_messages WHERE correlation_id = %s', (correlation_id,))
            cur.execute('DELETE FROM control_plane_outbox WHERE correlation_id = %s', (correlation_id,))
            cur.execute('DELETE FROM control_plane_commands WHERE correlation_id = %s', (correlation_id,))
    conn.commit()
""".strip()
    _run_command(
        [
...
from app.control_plane.application.delivery_service import DeliveryService
...
def _run_retry_path(base_url: str, compose_file: Path, runtime_dir: Path, context: ScenarioContext) -> None:
    ...
    decision = _record_processing_failure(
        ...
        source_stream="mc:control-plane:events:control_plane_run_submit_accepted:v1:p0",
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Mission Control control-plane smoke suite")
