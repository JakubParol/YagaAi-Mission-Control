...
export async function fetchRuns(params: {
  ...
  const response = await fetch(
    apiUrl(`/v1/control-plane/runs?${query.toString()}`),
  );
  if (!response.ok) {
    throw new Error(
      await parseErrorMessage(response, "Failed to load control-plane runs"),
    );
  }
...
export async function fetchTimeline(params: {
  ...
  const response = await fetch(
    apiUrl(`/v1/control-plane/timeline?${query.toString()}`),
  );
  if (!response.ok) {
    throw new Error(
...
export async function fetchAttempts(runId: string): Promise<RunAttempt[]> {
  const response = await fetch(
    apiUrl(`/v1/control-plane/runs/${runId}/attempts?limit=100`),
  );
  if (!response.ok) {
    throw new Error(
