...
export function normalizeCategory(event: TimelineUiEvent): FailureCategory | null {
  if (event.source === "attempt") {
    if (event.event_type === "control-plane.delivery.dead_lettered") {
      return "DEAD_LETTER";
    }
    if (event.event_type === "control-plane.delivery.retry_scheduled") {
      return "RETRY";
    }
  }
  ...
  for (const attempt of attempts) {
    if (attempt.retry_attempt > 1) {
      events.push({
        ...
        run_id: runId,
        run_status: runStatus,
        event_type: "control-plane.delivery.retry_scheduled",
        decision: "ACCEPTED",
    ...
    if (attempt.dead_lettered_at) {
      events.push({
        ...
        run_id: runId,
        run_status: runStatus,
        event_type: "control-plane.delivery.dead_lettered",
        decision: "REJECTED",
