  ...
  return (
    <div className="space-y-4">
      <PageShell
        icon={Radar}
        title="Run Timeline"
        subtitle="Inspect control-plane lifecycle events and delivery attempts"
        actions={
          <RefreshControl
            onRefresh={async () => {
              await loadRuns();
      ...
      {runsError ? (
        <ErrorCard
          ...
          message={runsError}
          suggestion="Verify API health and control-plane worker connectivity, then refresh. If this persists, inspect API logs for /v1/control-plane/runs and /timeline."
        />
      ) : null}
      ...
      {!runsLoading && runs.length === 0 ? (
        <EmptyState
          icon="default"
          title="No matching control-plane runs"
          description="No runs matched your filters. Clear status/time/run filters or submit a new control-plane command to generate telemetry."
        >
