...
const DEFAULT_COMMAND_TYPE = "control-plane.run.submit";
...
export function registerControlPlaneCommands(
  ...
  const run = program.command("run").description("control-plane run operations").showHelpAfterError();
  ...
  run
    ...
    .action(
      async (
        ...
        const payload = await ctx.client.post("/v1/control-plane/commands", {
          body: {
  ...
  run
    ...
    .action(async (opts: { runId: string }, command: Command) => {
      ...
      const payload = await ctx.client.get(
        `/v1/control-plane/runs/${encodeURIComponent(opts.runId)}`,
      );
  ...
  run
    .command("metrics")
    .description("show control-plane health metrics")
    .action(async (_opts: unknown, command: Command) => {
      const ctx = getContext(command);
      const payload = await ctx.client.get("/v1/control-plane/metrics");
  ...
  run
    ...
    .action(
      async (
        ...
        while (true) {
          const payload = await ctx.client.get("/v1/control-plane/timeline", {
            query: {
              run_id: opts.runId,
              status: opts.status,
