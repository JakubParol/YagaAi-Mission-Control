...
from app.orchestration.infrastructure.shared.mappers import (
...
_r = orchestration_runs.c
_t = orchestration_run_timeline.c
...
class DbReadModelRepository(ReadModelRepository):
    ...
    async def list_runs(
        ...
        *,
        ...
        count_result = await self._db.execute(
            select(count()).select_from(orchestration_runs).where(where)
        )
        ...
        causation_subq = (
            ...
            .limit(1)
            .correlate(orchestration_runs)
            .scalar_subquery()
            .label("causation_id")
        )
        query = (
            select(orchestration_runs, causation_subq)
            .where(where)
    ...
    async def list_timeline_entries(
        ...
        *,
        ...
        joined = orchestration_run_timeline.join(orchestration_runs, _t.run_id == _r.run_id)
        conditions = []
        if run_id is not None:
    ...
    async def list_run_attempts(
        ...
        *,
        ...
        joined = orchestration_outbox.join(
            orchestration_runs, _o.correlation_id == _r.correlation_id
        )
    ...
    async def get_health_snapshot(self) -> OrchestrationHealthSnapshot:
        ...
        wd_result = await self._db.execute(
            select(count())
            .select_from(orchestration_run_timeline)
            .where(
                and_(
                    _t.event_type == "orchestration.watchdog.action",
