...
from app.orchestration.infrastructure.shared.mappers import run_from_row, step_from_row
...
class DbRunRepository(RunRepository):
    ...
    async def get_run(self, *, run_id: str) -> OrchestrationRun | None:
        result = await self._db.execute(
            select(orchestration_runs).where(_r.run_id == run_id).limit(1)
        )
        row = result.first()
    ...
    async def create_run(self, *, run: OrchestrationRun) -> None:
        await self._db.execute(
            orchestration_runs.insert().values(
                run_id=run.run_id,
    ...
    async def get_step(self, *, run_id: str, step_id: str) -> OrchestrationStep | None:
        result = await self._db.execute(
            select(orchestration_run_steps)
    ...
    async def create_step(self, *, step: OrchestrationStep) -> None:
        await self._db.execute(
            orchestration_run_steps.insert().values(
    ...
    async def append_timeline_entry(self, *, entry: RunTimelineEntry) -> None:
        await self._db.execute(
            orchestration_run_timeline.insert().values(
                id=entry.id,
                run_id=entry.run_id,
