...
class WatchdogService:
    ...
    async def evaluate_stale_runs(
        ...
        for run in runs:
            ...
            updated = await self._repo.apply_watchdog_action_if_lease_matches(
                ...
                last_event_type=f"orchestration.watchdog.{action.value.lower()}",
    ...
    def _detect_reason(self, *, run: OrchestrationRun, now_dt: datetime) -> str | None:
        ...
        if run.last_heartbeat_at:
            ...
            if heartbeat_age > settings.orchestration_watchdog_heartbeat_grace_seconds:
    ...
    def _choose_action(self, *, run: OrchestrationRun, reason_code: str) -> WatchdogAction:
