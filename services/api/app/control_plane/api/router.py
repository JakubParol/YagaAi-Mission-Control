...
from app.control_plane.api.schemas import (
    ControlPlaneHealthMetricsResponse,
    EnvelopePayload,
    RunAttemptResponse,
    RunStateResponse,
...
def _ensure_capability_enabled(*, enabled: bool, capability: str) -> None:
    ...
    raise HTTPException(
        ...
        detail=f"Control-plane capability disabled: {capability}",
    )
...
async def submit_command(
    ...
    _ensure_capability_enabled(
        enabled=settings.control_plane_commands_enabled,
        capability="control-plane.commands",
    )
...
async def watchdog_sweep(
    ...
    _ensure_capability_enabled(
        enabled=settings.control_plane_watchdog_enabled,
        capability="control-plane.watchdog",
    )
...
async def get_control_plane_metrics(
    service: RunReadModelService = Depends(get_run_read_model_service),
) -> Envelope[ControlPlaneHealthMetricsResponse]:
    metrics = await service.get_health_metrics()
    return Envelope(data=_to_control_plane_metrics_response(metrics))
...
def _to_timeline_entry_response(entry: TimelineEntryReadModel) -> TimelineEntryResponse:
    ...
    is_watchdog_action = entry.event_type == "control-plane.watchdog.action"
...
def _to_control_plane_metrics_response(
    metrics: ControlPlaneHealthMetrics,
) -> ControlPlaneHealthMetricsResponse:
    return ControlPlaneHealthMetricsResponse(
