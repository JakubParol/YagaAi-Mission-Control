...
from app.control_plane.infrastructure.shared.mappers import outbox_event_from_row
from app.control_plane.infrastructure.tables import (
    control_plane_commands,
    control_plane_outbox,
)

_c = control_plane_commands.c
...
class DbCommandRepository(CommandRepository):
    ...
    async def create_command_with_outbox(
        ...
        *,
        ...
        await self._db.execute(
            control_plane_commands.insert().values(
                id=command.id,
        ...
        await self._db.execute(
            control_plane_outbox.insert().values(
                id=outbox_event.id,
    ...
    async def get_outbox_event(self, *, outbox_event_id: str) -> OutboxEventEnvelope | None:
        result = await self._db.execute(
            select(control_plane_outbox).where(_o.id == outbox_event_id).limit(1)
        )
        row = result.first()
        if row is None:
            return None
