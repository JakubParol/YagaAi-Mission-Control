...
from app.control_plane.application.ports import ConsumerRepository, RunRepository
...
_RUN_EVENT_TO_STATUS: dict[str, RunStatus] = {
    "control-plane.run.submit.accepted": RunStatus.PENDING,
    "control-plane.run.started": RunStatus.RUNNING,
    "control-plane.run.succeeded": RunStatus.SUCCEEDED,
    "control-plane.run.failed": RunStatus.FAILED,
    "control-plane.run.cancelled": RunStatus.CANCELLED,
}
_STEP_EVENT_TO_STATUS: dict[str, StepStatus] = {
    "control-plane.step.started": StepStatus.RUNNING,
    "control-plane.step.succeeded": StepStatus.SUCCEEDED,
    "control-plane.step.failed": StepStatus.FAILED,
    "control-plane.step.cancelled": StepStatus.CANCELLED,
    "control-plane.step.skipped": StepStatus.SKIPPED,
}
...
class WorkerStateMachineService:
        ...
        *,
        ...
        if duplicate:
            log_event(
                self._logger,
                level=logging.INFO,
                event="control-plane.worker.duplicate_message",
                run_id=run_id,
                message_id=message_id,
        ...
        log_event(
            self._logger,
            level=logging.INFO,
            event="control-plane.worker.transition_applied",
            run_id=run_id,
            event_type=event_type,
            message_id=message_id,
        ...
        for run in in_flight_runs:
            await self._run_repo.append_timeline_entry(
                entry=RunTimelineEntry(
                    ...
                    message_id=None,
                    event_type="control-plane.run.reconciled",
        ...
        if run is None:
            if target_status != RunStatus.PENDING:
                return (
                    ...
                    "RUN_NOT_FOUND",
                    "Run must be created by control-plane.run.submit.accepted",
                )
        ...
        if step is None:
            if target_status != StepStatus.RUNNING:
                return (
                    TransitionDecision.REJECTED,
                    "STEP_NOT_FOUND",
                    "Step must be created by control-plane.step.started",
                )
    ...
    def _default_timeout_at(self, occurred_at: str) -> str:
        ...
        return (
            (base + timedelta(seconds=settings.control_plane_watchdog_default_timeout_seconds))
            .isoformat()
            .replace(
                "+00:00",
                "Z",
