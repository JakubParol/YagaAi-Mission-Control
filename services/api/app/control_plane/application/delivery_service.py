...
from app.control_plane.application.ports import CommandRepository
...
class DeliveryService:
    def __init__(self, repo: CommandRepository) -> None:
        self._repo = repo
        self._base_backoff_seconds = settings.control_plane_retry_base_backoff_seconds
        self._max_backoff_seconds = settings.control_plane_retry_max_backoff_seconds
        self._dead_letter_stream = (
            f"{settings.control_plane_stream_prefix}:dead-letter:v"
            f"{settings.control_plane_stream_version}"
        )
    ...
    async def record_processing_failure(
        ...
        *,
        ...
        if event is None:
            raise NotFoundError(f"Control-plane outbox event not found: {outbox_event_id}")
        ...
        if next_attempt <= event.max_attempts:
            ...
            log_event(
                ...
                event="control-plane.delivery.retry_scheduled",
                outbox_event_id=outbox_event_id,
        ...
        log_event(
            self._logger,
            level=logging.ERROR,
            event="control-plane.delivery.dead_lettered",
            outbox_event_id=outbox_event_id,
            correlation_id=event.correlation_id,
            max_attempts=event.max_attempts,
