# Orchestration Redis Stream Contract (MC-371)

Defines the v1 Redis stream topology and consumer-group naming used by orchestration producers and workers.

## Contract version

- Stream version: `v1`
- Prefix: `mc:orchestration` (configurable with `MC_API_ORCHESTRATION_STREAM_PREFIX`)
- Partition count: `8` by default (`MC_API_ORCHESTRATION_STREAM_PARTITIONS`)
...
`mc:orchestration:commands:<topic>:v1:p<partition>`

Example:

`mc:orchestration:commands:orchestration_run_submit:v1:p3`

### Event stream (partitioned)

Pattern:

`mc:orchestration:events:<topic>:v1:p<partition>`

Example:

`mc:orchestration:events:orchestration_run_submit_accepted:v1:p3`

### Dead-letter stream (unpartitioned)

Pattern:

`mc:orchestration:dead-letter:v1`
...
- Worker group: `orchestration-workers-v1` (`MC_API_ORCHESTRATION_WORKER_CONSUMER_GROUP`)
- Watchdog group: `orchestration-watchdog-v1` (`MC_API_ORCHESTRATION_WATCHDOG_CONSUMER_GROUP`)
