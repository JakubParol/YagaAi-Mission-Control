...
def test_watchdog_sweep_returns_503_when_capability_disabled(client, monkeypatch) -> None:
    monkeypatch.setattr(
        "app.orchestration.api.router.settings.orchestration_watchdog_enabled", False
    )
    ...
    assert response.json()["detail"] == "Orchestration capability disabled: orchestration.watchdog"
