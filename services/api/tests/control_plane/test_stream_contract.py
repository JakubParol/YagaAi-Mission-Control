...
from app.control_plane.domain.stream_contract import RedisStreamContract, partition_for_key


def _contract(partitions: int = 8) -> RedisStreamContract:
    return RedisStreamContract(
        prefix="mc:control-plane",
        version=1,
        partitions=partitions,
        worker_consumer_group="control-plane-workers-v1",
        watchdog_consumer_group="control-plane-watchdog-v1",
    )
...
def test_command_stream_uses_partitioned_contract_shape() -> None:
    ...
    key = contract.command_stream("control-plane.run.submit", "corr-123")
    assert key.startswith("mc:control-plane:commands:control-plane_run_submit:v1:p")
    assert key.count(":p") == 1


def test_event_stream_uses_partitioned_contract_shape() -> None:
    contract = _contract(partitions=4)
    key = contract.event_stream("control-plane.run.submit.accepted", "corr-123")
    assert key.startswith("mc:control-plane:events:control-plane_run_submit_accepted:v1:p")
    assert key[-1] in {"0", "1", "2", "3"}


def test_dead_letter_stream_contract_is_unpartitioned() -> None:
    contract = _contract()
    assert contract.dead_letter_stream == "mc:control-plane:dead-letter:v1"
