...
def _seed_command(conn: psycopg.Connection, *, command_id: str, correlation_id: str) -> None:
    conn.execute(
        """
...
def _seed_run(conn: psycopg.Connection, *, run_id: str, status: str, correlation_id: str) -> None:
    conn.execute(
        """
...
def _seed_timeline_entry(
    ...
    *,
    ...
    conn.execute(
        """
        INSERT INTO control_plane_run_timeline(
...
def test_run_state_endpoints_include_identifier_contract(client, db_path: str) -> None:
    ...
    list_response = client.get("/v1/control-plane/runs", params={"status": "RUNNING"})
    ...
    get_response = client.get("/v1/control-plane/runs/run-1")
...
def test_timeline_endpoint_supports_filters_and_deterministic_pagination(
    ...
    page1 = client.get("/v1/control-plane/timeline", params={"limit": 2, "offset": 0})
    assert page1.status_code == 200
    page1_data = page1.json()["data"]
    assert [entry["id"] for entry in page1_data] == ["t-4", "t-3"]

    page2 = client.get("/v1/control-plane/timeline", params={"limit": 2, "offset": 2})
    assert page2.status_code == 200
    page2_data = page2.json()["data"]
    assert [entry["id"] for entry in page2_data] == ["t-2", "t-1"]

    filtered = client.get(
        "/v1/control-plane/timeline",
        params={
            "run_id": "run-1",
            "status": "RUNNING",
            "event_type": "control-plane.watchdog.action",
...
def test_run_attempts_endpoint_returns_attempts_and_not_found(client, db_path: str) -> None:
    with pg_connect(db_path) as conn:
        ...
        conn.execute(
            ...
            [
                "out-1",
                "cmd-1",
                "control-plane.run.submit.accepted",
                "1.0",
    ...
    response = client.get("/v1/control-plane/runs/run-1/attempts")
    ...
    not_found = client.get("/v1/control-plane/runs/run-missing/attempts")
    assert not_found.status_code == 404
    assert not_found.json()["error"]["code"] == "NOT_FOUND"


def test_control_plane_metrics_endpoint_returns_queue_and_latency_metrics(
    client, db_path: str
) -> None:
    with pg_connect(db_path) as conn:
    ...
    response = client.get("/v1/control-plane/metrics")
    assert response.status_code == 200
